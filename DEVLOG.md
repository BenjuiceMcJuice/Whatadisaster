...
| 2026-07-19 | **UI/UX P1 (game-screen density) shipped** — the "too busy" game screen rebuilt per Phase 2 of the assessment: slim status bar + single consequence pill; Consequence Tracker/Activated Agencies/Command Level demoted into a tap-to-open status sheet; situation panel collapsible → sticky one-line recap; calmed motion (removed 3 simultaneous pulses/glow, red reserved for urgency); stat chips capped at 3 + "+N more"; `prefers-reduced-motion` added. Game screen ~2.6 → ~1.0–1.3 screenfuls at 390×844 (≤1.6 target met). Both scenarios verified start→results, 0 JS errors. Content/scoring untouched. | ✅ Done |
